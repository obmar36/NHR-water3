...
const GUIDANCE = [
  {
    ...
    detail: '夜間需求約 8,000 CMD，以 100HP 組（P1@57 主底 + P2@51 輔助）承接即足；可停日間保底之 150HP_P3。100HP 在現場狀況下健康、效率約 70%，遠高於已衰退的 150HP（約 54%）。',
    action: '切換夜間調度', cfg: { P3_100HP: 57, P4_100HP: 51 }, save: 39, savekw: 82,
    steps: ['停 150HP_P3，保留 100HP_P1@57Hz 為夜間主底', '100HP_P2@51Hz 依配水池水位輔助投入', '確認配水池回升速度足夠；不足時 P1 可提升至 60Hz'],
  },
  {
    id: 'R-RULE', level: 'info', dev: 'P3 / P4', t: '法則', status: '待處理', kind: 'dispatch',
    title: '確定性節能法則 · 以 P3@55 取代 P4@60',
    short: '日間用 P3 取代 P4 更省電',
    brief: '同樣出水量，P3@55Hz 比 P4@60Hz 少用約 15% 的電。',
    detail: '相近出水量下，P3@55Hz（約 3,797 CMD）較 P4@60Hz（約 4,430 CMD）少約 15% 輸入功率；且 P4 現場已衰退（−2,539 CMD vs 出廠）。日間高流量一律以 P3 保底、P4 列備用。',
    action: '套用 P3@55Hz 保底', cfg: { P2_150HP: 55, P3_100HP: 57, P4_100HP: 51 }, save: 15, savekw: 14,
    steps: ['日間以 150HP_P3@55Hz 保底，不啟用已衰退之 P4', '再依需求疊加 100HP_P1 / P2', '需求跨過 2,600 CMD 時 150HP 才考慮，並優先選 P3'],
  },
  {
    id: 'R-P3HZ', level: 'warn', dev: 'P3_100HP', t: '待確認', status: '待確認', kind: 'note',
    title: '夜間 P1 頻率 57 / 60Hz 版本待定版',
    detail: '報告控制邏輯與 P1 峰值效率點皆為 57Hz（SEC 0.387、效率 70.2%），平台暫定 57Hz；惟時段配置圖另出現 60Hz 保底版本。57Hz 較省電，但補水不足時需升 60Hz。',
...
const CHAT_CANNED = {
  '哪一台最該停、哪一台最該開？': {
    a: '夜間應「停 150HP、開 100HP」：100HP（P1/P2）現場效率約 70%，遠高於已衰退的 150HP（約 54%）。日間高流量才以 P3@55Hz 保底、P4 列備用。同馬力擇優以效率交叉點為界：150HP@2,600、100HP@3,400 CMD。',
    src: [{ doc: '4_效率交叉與選泵', loc: '交叉點/選泵法則' }, { doc: '3_現場效能曲線', loc: 'P1–P4 40–60Hz' }],
  },
  '夜間 P1 要用 57 還是 60Hz？': {
    a: '平台暫定 57Hz：與報告控制邏輯及 P1 峰值效率點一致（57Hz：SEC 0.387、泵效率 70.2%），較 60Hz 省電。但時段配置圖另有 60Hz 保底版本，且配水池夜間補水不足時允許升至 60Hz。57/60Hz 差異尚待向廠商／現場確認後定版。',
